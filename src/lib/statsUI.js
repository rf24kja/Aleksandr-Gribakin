...
export function renderDashboard(agg, t = {}, sortMode = 'default', toolTotal = 0) {
  ...
  const topTech = agg.tech.slice(0, 6).map((x) => ({
    label: x.label,
    value: x.value,
    color: 'var(--sb-accent)',
  }));
  ...
  const sorts = [
    ...
    ['scale', t.SORT_SCALE || 'By scale'],
    ['stack', t.SORT_STACK || 'By stack depth'],
  ];

  return `<div class="pdash-head">
      <span class="pdash-title">${esc(t.TITLE || 'Selection at a glance')}</span>
      <div class="pdash-sort" role="group" aria-label="${esc(t.SORT || 'Sort')}">
        ${sorts
          .map(
            ([id, label]) =>
              `<button type="button" class="pdash-sort-btn${id === sortMode ? ' active' : ''}"
                       data-sort="${id}" aria-pressed="${id === sortMode}">${esc(label)}</button>`,
          )
          .join('')}
      </div>
    </div>
    <div class="pdash-tiles">
      ${tiles
        .map(
          (tile) => `<div class="pdash-tile">
            <span class="pdash-num" data-display="${esc(String(tile.value))}">${esc(String(tile.value))}</span>
            <span class="pdash-cap">${esc(tile.label)}</span>
          </div>`,
        )
        .join('')}
    </div>
    <div class="pdash-tech">
      <div class="pdash-tech-title">${esc(t.TOP_TECH || 'Most used in this selection')}</div>
      ${renderBars(topTech, { compact: true })}
    </div>`;
}

/** Fills dashboard bars in after render. */
export function animateDashboard(root) {
  const instant = REDUCED_MOTION();
  root.querySelectorAll('.sb-fill').forEach((fill, i) => {
    const w = fill.style.getPropertyValue('--sb-w');
    fill.style.width = '0';
    if (instant) { fill.style.width = w; return; }
    setTimeout(() => { fill.style.width = w; }, 60 + i * 40);
  });
}
